 ...
 * Description:
 *
 * The M256 UART Library. (UART = "Universal Aynchronous Receiver Transceiver")
 *
 * It contains data transfer functions for the UART (0). 
 * Reception is Interrupt based with a circular buffer.
 *
 * --------
 *
...
/**
 * Write a 32 Bit number (with specified base) to the UART.
 * Usage is the same as writeInteger.
 */
void write32BitInteger(int32_t number, uint8_t base)
{
	char buffer[33];
...
{	
	...
	if(((uint16_t)(write_size - read_size)) < UART_RECEIVE_BUFFER_SIZE) {
		uart_receive_buffer[write_pos++] = UDR0;
		write_size++;
...
{
	uart_status = UART_BUFFER_OK;
	if(((uint16_t)(write_size - read_size)) > 0) {
		read_size++;
 ...
 */
uint16_t readChars(char *buf, uint16_t numberOfChars)
{
	uint16_t i = 0;
	uart_status = UART_BUFFER_OK;
	while(((uint16_t)(write_size - read_size)) > 0 && i < numberOfChars) {
		read_size++;
...
{
	return (((uint16_t)(write_size - read_size)));
}
...
void clearReceptionBuffer(void)
{
	...
	UCSR0B &= ~(1 << RXCIE0); // disable UART RX Interrupt
	dummy = UDR0;
	read_pos = 0;
	write_pos = 0; 
	read_size = 0;
	write_size = 0;
	uart_status = UART_BUFFER_OK;
	UCSR0B |= (1 << RXCIE0); // enable Interrupt again
}
