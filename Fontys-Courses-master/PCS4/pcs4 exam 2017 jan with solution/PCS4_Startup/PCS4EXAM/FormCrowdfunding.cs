...
namespace PCS4EXAM
{
    public partial class FormCrowdfunding : Form
    {
        ...
        private void AddStartUp(StartUp s)
        {
            if (s.Description.ToLower().Contains("music")) s.ReachedThreshold += new StartUp.ReachedThresholdHandler(frmCapitalStudio.ShowStartupInMyListbox);
            if (s.Threshold >= 10000) s.ReachedThreshold += new StartUp.ReachedThresholdHandler(frmRedFlat.AlertMe);
            s.ReachedThreshold += new StartUp.ReachedThresholdHandler(congratulations);
            this.cf.addStartup(s);
        }
        private void btnAddDonation_Click(object sender, EventArgs e)
        {
            ...
            if (s != null)
            {
                ...
                s.addDonation(tbNameDonator.Text, amount);
                linkedList.addDonationToLinkedList(tbNameStartup.Text, tbNameDonator.Text, amount);
                this.showStartups();
            }
            else
            {
        ...
        private void btnShowLinkedList_Click(object sender, EventArgs e)
        {
            ...
            Node current = linkedList.FirstNode;
            while(current != null)
            {
